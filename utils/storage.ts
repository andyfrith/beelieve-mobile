...
export async function getMedications(): Promise<Medication[]> {
  try {
    ...
    // console.log("getMedications() data", data);
...
export async function addMedication(medication: Medication): Promise<void> {
  try {
    // console.log("addMedication() medication", medication);
    const medications = await getMedications();
    medications.push(medication);
    // console.log("addMedication() medications", medications);
  ...
  try {
    ...
    if (taken) {
      ...
      if (
        ...
        Number(medication.currentSupply) > 0
      ) {
        medication.currentSupply = Number(medication.currentSupply) - 1;
